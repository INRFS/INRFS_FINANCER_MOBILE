...
const collectionActionButton = { flexGrow: 1, flexBasis: "45%" as const };

export function AdminCollectionsScreen() {
  const load = useCallback(async () => {
    const [collections, users, settings] = await Promise.all([
      platformApi.collections.all(),
      platformApi.admin.allUsers().catch(() => ({ items: [] })),
  ...
  const submit = async () => { if (!selected || !action || saving) return; if(action==="assign"&&!agentId)return Alert.alert("Select an agent");if(["promise","followup","payment"].includes(action)&&!isValidDateOnly(date))return Alert.alert("Invalid date","Enter a real date as YYYY-MM-DD.");if(["promise","followup"].includes(action)&&date<today())return Alert.alert("Invalid date","Choose today or a future date.");if(action==="payment"&&(Number(amount)<=0||Number(amount)>actionableDue(selected)))return Alert.alert("Invalid amount",`Enter an amount up to ${money(actionableDue(selected))}.`);if(action==="payment"&&date>today())return Alert.alert("Invalid payment date","Payment date cannot be in the future.");if(action==="call"&&!notes.trim())return Alert.alert("Call notes required"); setSaving(true); try {
  ...
  } catch (e) { Alert.alert("Action not saved", message(e)); } finally { setSaving(false); } };
  ...
  return <Screen><Header title="Collections" subtitle="Due and overdue work queue" action={<View style={{flexDirection:"row",gap:6}}><Button label="Export" variant="secondary" onPress={()=>void shareCsv(`collection-operations-${today()}.csv`,rows).catch(e=>Alert.alert("Export failed",message(e)))}/><Button label="Refresh" variant="ghost" onPress={() => void refresh()}/></View>}/>
    ...
    {rows.map((item: any) => <Card key={item.id}><Badge status={displayStatus(item)}/><DataRow title={item.customer} subtitle={`${item.financer ?? item.financerName} · ${item.loanNumber} · Due ${String(item.dueDate).slice(0, 10)}`} amount={money(actionableDue(item))}/><DataRow title="Assigned agent" amount={item.assignedToName ?? "Unassigned"}/><View style={{ flexDirection: "row", flexWrap: "wrap", gap: 7 }}><Button style={collectionActionButton} label="Assign" variant="secondary" onPress={() => open(item, "assign")}/><Button style={collectionActionButton} label="Promise" variant="secondary" onPress={() => open(item, "promise")}/><Button style={collectionActionButton} label="Follow-up" variant="secondary" onPress={() => open(item, "followup")}/><Button style={collectionActionButton} label="Call" variant="secondary" onPress={() => open(item, "call")}/><Button style={collectionActionButton} label="Payment" onPress={() => open(item, "payment")}/><Button style={collectionActionButton} label="SMS" variant="ghost" onPress={() => void remind(item)}/><Button style={collectionActionButton} label="WhatsApp" variant="ghost" onPress={() => void whatsapp(item)}/></View>{(item.activities ?? []).map((activity: any) => <Text key={activity.id}>{activity.type}: {activity.notes}</Text>)}</Card>)}
    {action ? <Card><Header title={action === "assign" ? "Assign agent" : action === "payment" ? "Record payment" : action === "call" ? "Record call" : action === "promise" ? "Promise to pay" : "Schedule follow-up"}/>{action === "assign" ? data.agents.map((agent: any) => <Button key={agent.id} label={`${agent.firstName ?? ""} ${agent.lastName ?? ""}`.trim() || agent.email} variant={agentId === agent.id ? "primary" : "secondary"} accent="purple" onPress={() => setAgentId(agent.id)}/>) : null}{action === "call" ? <><ScrollView horizontal><View style={{ flexDirection: "row", gap: 8 }}>{["Customer contacted","No answer","Busy / switched off","Payment confirmed","Promise to pay","Dispute raised"].map(x => <Button key={x} label={x} variant={outcome === x ? "primary" : "secondary"} accent="purple" onPress={() => setOutcome(x)}/>)}</View></ScrollView><Button label="Open phone dialer" variant="secondary" onPress={() => void Linking.openURL(`tel:${selected?.customerPhone ?? ""}`)}/></> : null}{action !== "assign" && action !== "call" ? <Field label={action === "payment" ? "Payment date" : "Date"} value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" maxLength={10}/> : null}{action === "call" && outcome === "Promise to pay" ? <Field label="Promise date" value={date} onChangeText={setDate} maxLength={10}/> : null}{action === "call" ? <Field label="Next follow-up (optional)" value={followUpDate} onChangeText={setFollowUpDate} maxLength={10}/> : null}{action === "payment" ? <><Field label="Amount" value={amount} onChangeText={setAmount} keyboardType="decimal-pad"/><ScrollView horizontal><View style={{ flexDirection: "row", gap: 8 }}>{["Cash","Upi","BankTransfer","Cheque","Card","Other"].map(x => <Button key={x} label={x} variant={mode === x ? "primary" : "secondary"} accent="purple" onPress={() => setMode(x)}/>)}</View></ScrollView><Field label="Reference" value={reference} onChangeText={setReference}/></> : null}{action !== "assign" && action !== "payment" ? <Field label="Notes" value={notes} onChangeText={setNotes} multiline/> : null}<View style={{ flexDirection: "row", gap: 8 }}><Button label="Cancel" variant="secondary" disabled={saving} onPress={() => setAction(null)}/><Button label="Confirm" accent="purple" loading={saving} onPress={() => void submit()}/></View></Card> : null}
  </Screen>;
}
